...
for focus_sample in ["section4"]:
    ...
    ad_merged.obs["domain"] = np.concatenate(
        [[j] * i.shape[0] for i, j in zip(atlas_adata, atlasname)]
    )
    ...
    x, y = (
        X1_raw[:, X1_raw.var.index == focus_gene].X.reshape(
            -1,
        ),
        average_features,
    )
    ...
    pd.DataFrame(dic, index=range(1)).to_csv(
        f"source_data/baseline1/{focus_sample}_{focus_gene}.csv"
    )
