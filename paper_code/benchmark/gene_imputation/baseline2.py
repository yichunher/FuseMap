...
for focus_sample in ["section4"]:
    ...
    combine_adata = X1_subset.concatenate(
        scrna_subset, batch_key="dataset", batch_categories=["st", "scrna"]
    )
    ...
    gene_raw = (
        ad_merged_raw[:, ad_merged_raw.var.index == focus_gene].X.toarray().flatten()
    )
    ...
    n1_cells = np.array(
        ad_merged_starmap.obsm["X_umap"]
    )  # Replace with your n1 cells data
    n2_cells = np.array(
        ad_merged_other.obsm["X_umap"]
    )  # Replace with your n2 cells data
    ...
    x, y = (
        X1[:, X1.var.index == focus_gene].X.reshape(
            -1,
        ),
        average_features,
    )
    ...
    pd.DataFrame(dic, index=range(1)).to_csv(
        f"source_data/baseline2/{focus_sample}_{focus_gene}.csv"
    )
