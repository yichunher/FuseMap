...
def count_cell_type_contacts(
    neighbors,
    cell_type_ids_of_cells,
    N_cell_types,
    permutated_cell_ids=None,
...
def generate_cell_type_contact_count_matrices(
    df,
    cell_type_col,
    coord_cols,
    cell_types,
    ...
    cell_type_id_map = pd.DataFrame(
        {"cell_type_id": cell_type_ids, "cell_type": cell_types}
    )
    cell_type_ids_of_cells = list(
        df[[cell_type_col]].merge(
            cell_type_id_map, left_on=cell_type_col, right_on="cell_type", how="left"
        )["cell_type_id"]
    )

    if permutation_method == "no_permutation":
        return count_cell_type_contacts(
            neighbors_contact, cell_type_ids_of_cells, N_cell_types
        )
        ...
        for i in range(N_permutations):
            permutated_cell_ids = generate_global_permutation(
                len(cell_type_ids_of_cells)
            )
            permuted_ct_contact_count_mtx = count_cell_type_contacts(
                neighbors_contact,
                cell_type_ids_of_cells,
                N_cell_types,
                permutated_cell_ids=permutated_cell_ids,
            )
