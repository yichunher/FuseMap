...
def preprocess_raw( X_input, kneighbor, input_identity, use_input, n_atlas, data_pth=None):
    print('---------------------------------- Preprocess adata ----------------------------------')
    X_input = preprocess_adata(X_input,n_atlas)

    print('---------------------------------- Construct graph adata ----------------------------------')
    construct_graph(X_input, n_atlas,kneighbor, input_identity)

    print('---------------------------------- Process graph adata ----------------------------------')
        ...
        if input_identity[i_atlas] == 'ST':
            if not 'adj_normalized' in adatas[i_atlas].obsm:
                ...
                if k == 'delaunay':
                    ...
                    adjacency_matrix = csr_matrix((np.ones_like(indices, dtype=np.float64), indices, indptr),
                ...
                if 'knn' in k:
                    ...
                    knn_k = 10
                    nbrs = NearestNeighbors(n_neighbors=knn_k + 1, algorithm='auto').fit(data)
                    ...
                    for i in range(num_spots):
                        ...
                        for j in indices[i, 1:]:
                            ...
                            adjacency_matrix[j, i] = 1  # Because it's an undirected graph
    ...
    for i in range(n_atlas):
        if input_identity[i] == 'ST':
            if not 'adj_normalized' in adatas[i].obsm:
                ...
                adata.obsm['adj_normalized'] = adj_normalized  # sparse_mx_to_torch_sparse_tensor(adj_normalized)
